...
public class CreateNewBudget extends AppCompatActivity {
    ...
    private SQLiteDatabase budgetDatabase;
    ...
    private void createNewBudget() {
        ...
        cv.put(ShowBudgets.BudgetEntry.TARGET_SPENDING_COLUMN, target);
        cv.put(ShowBudgets.BudgetEntry.BUDGET_NOTES, notes);
        cv.put(ShowBudgets.BudgetEntry.TOTAL_SPENT, "0"); //Set values on create to 0
        cv.put(ShowBudgets.BudgetEntry.REMAINDER, "0");
        cv.put(ShowBudgets.BudgetEntry.ADD_COLUMN, "0");
        cv.put(ShowBudgets.BudgetEntry.REMOVE_COLUMN, "0");

        //insert values into database
        budgetDatabase.insert(ShowBudgets.BudgetEntry.BUDGETS_LIST_TABLE_NAME, null, cv);
